...
public:
  ...
  template <typename Component> sparse_array<Component> &register_component() {
    ...
    return std::any_cast<sparse_array<Component> &>(
        _components_arrays[typeIdx]);
  }

  template <typename Component> sparse_array<Component> &get_components() {
    std::type_index typeIdx(typeid(Component));
  ...
  add_component(const entity_t &entity, Component &&component) {
    auto &array = get_components<Component>();
    return array.insert_at(static_cast<size_t>(entity),
                           std::forward<Component>(component));
  }

  template <typename Component, typename... Params>
  typename sparse_array<Component>::value_type &
  emplace_component(const entity_t &entity, Params &&...params) {
    auto &array = get_components<Component>();
    return array.emplace_at(static_cast<size_t>(entity),
                            std::forward<Params>(params)...);
  }

  template <typename Component> void remove_component(const entity_t &entity) {
    auto &array = get_components<Component>();
    array.erase(static_cast<size_t>(entity));
  }

  template <typename Function> void add_system(Function &&system) {
...
private:
  ...
  std::unordered_map<std::type_index,
                     std::function<void(registry &, const entity_t &)>>
      _erase_functions;
